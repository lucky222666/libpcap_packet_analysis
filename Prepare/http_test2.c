...
void got_packet(u_char *args,const struct pcap_pkthdr *header,const u_char *packet)
{
    ...
    if ((mysql_real_connect(&conn, "localhost", "root", "", "Capture", 0, NULL, 0)) &&  (ntohs(tcp->th_sport)==80||ntohs(tcp->th_dport)==80)) {
        ...
        sprintf(insert_query, "insert into http_test values('%d','%s','%s','%d','%d','%d','%d','')", \
		...
		if(ntohs(tcp->th_dport)==80){
			FILE *out;
			out = fopen("http_request.txt","ab+");      //读写打开或建立一个二进制文件，允许读和写
			if(out == NULL){
					exit(EXIT_FAILURE);
			}
...
int main(int argc, char **argv)
{
    ...
    // FILE * input = fopen("http_request.txt","a+");    
    ...
    //     fseek(input,0,SEEK_SET);//将指针置于文件开头
    //     if((validCount = fread(buffer,sizeof(buffer),5,input))!=0){
