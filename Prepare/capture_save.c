...
void got_packet(u_char *args,const struct pcap_pkthdr *header,const u_char *packet)
{
    ...
    if ((mysql_real_connect(&conn, "localhost", "root", "", "Capture", 0, NULL, 0)) &&  (ntohs(tcp->th_sport)==80||ntohs(tcp->th_dport)==80)) {
        ...
        sprintf(insert_query, "insert into prepare values('%d','%s','%s','%d','%d','%d','%d','%s')", \
        ...
        // char tmppayload[1518];
        // printf("有效载荷是 %s\n",payload);
        // //spintf(tmppayload,payload);
        // //puts(tmppayload);
        // strcat(insert_query, tmppayload);
        // strcat(insert_query, ")");
        ...
        fprintf(stderr, "Connection failed or is not HTTP packet\n");
        if (mysql_errno(&conn)) {
